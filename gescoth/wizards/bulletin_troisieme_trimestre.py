	...
	def imprimer_bulletin_troisieme_trimestre(self):
		...
		for el in eleve_ids:
			eleve = {
				...
				'conduite':el.eleve_id.afficher_conduite(self.annee_scolaire_id.id,'s3'),
			}
			el_note_ids = self.env['gescoth.note'].search([('eleve_id','=', el.eleve_id.id),('saison','=','s3'),('annee_scolaire','=', self.annee_scolaire_id.id)])
		...
		for el in liste_note:
			moyenne_annuel = (el['premier_trimestre']['moyenne'] + el['second_trimestre']['moyenne'] + el['moyenne_sur_vingt'])/3
			toute_les_note_annuelle.append(round(moyenne_annuel,2))


		for el in liste_note:
			# raise ValidationError(_(liste_note[0]['premier_trimestre']))
			moyenne_annuel = round(((el['premier_trimestre']['moyenne'] + el['second_trimestre']['moyenne'] + el['moyenne_sur_vingt'])/3),2)

			# raise ValidationError(_(el['second_trimestre']['moyenne']))

			rang = Rangb(round(moyenne_annuel,2),el['sexe'],toute_les_note_annuelle)
