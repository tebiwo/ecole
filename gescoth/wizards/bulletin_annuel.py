...
class BulletinAnulle(models.TransientModel):
    _name = 'gescoth.bulletin.annuel'
    _description = 'Bulletin annuel'
    ...
    def imprimer_bulletin_annuel(self):
        data = {}
        liste_note = []
        liste_note_total = []
        ...
        for el in eleve_ids:
            ...
            el_note_ids = self.env['gescoth.note.annuelle'].search([('eleve_id','=', el.eleve_id.id) ,('annee_scolaire','=', self.annee_scolaire_id.id)])
            ...
            for note in el_note_ids:
                ...
                vals_note = {
                    ...
                    'sequence1': note.note_sequence1,
                    's1_est_compose': not note.s1_is_not_compose,
                    'sequence2': note.note_sequence2,
                    's2_est_compose': not note.s2_is_not_compose,
                    'sequence3': note.note_sequence3,
                    's3_est_compose': not note.s3_is_not_compose,
                    'sequence4': note.note_sequence4,
                    's4_est_compose': not note.s4_is_not_compose,
                    'sequence5': note.note_sequence5,
                    's5_est_compose': not note.s5_is_not_compose,
                    'sequence6': note.note_sequence6,
                    's6_est_compose': not note.s6_is_not_compose,
                    'coef':note.coeficient_id.coef,
                    'total': note.total,
                    'moyenne_annuelle': note.moyenne_annuel,
                    'rang':note.rang,
                ...
                if note.coeficient_id.matiere.type_matiere == 'sport' and not el.Apt_sport:
                    ...
                    if not note.coeficient_id.est_facultative or not note.is_not_compose:
                        coef += note.coeficient_id.coef
                    total += (note.moyenne_annuel * note.coeficient_id.coef)
        ...
        # raise ValidationError(_(liste_note_total))
        ...
        for el in liste_note:
            moyenne_annuel = round( el['moyenne_sur_vingt'],2)
        ...
        for el in liste_note:
            ...
            exist = self.env['gescoth.examen.annuel.resultat'].search([
                ('eleve_id','=',el['id']),
                ('classe_id','=',self.classe_id.id),
                ('nombre','=',6),
                ('annee_scolaire','=', self.annee_scolaire_id.id,)
            ...
            if exist:
                if exist.state == '0':
                    exist.unlink()
                    self.env['gescoth.examen.annuel.resultat'].create(vals)
            else:
                self.env['gescoth.examen.annuel.resultat'].create(vals)

        return self.env.ref('gescoth.bulletin_par_sequence_annuel_report_view').report_action(self, data=data)
