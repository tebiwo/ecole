...
class GescothEleve(models.Model):
	...
	nom_eleve = fields.Char(string="Nom et prénom(s)", store=True, copy=True, compute="onchange_nom_famille")		
	...
	date_naissance = fields.Date(string="Date de naissance",track_visibility='always', required=True )
	...
	classe = fields.Many2one('gescoth.classe', string="Classe en cours", required=True )
	...
	def create(self, vals):
		...
		if vals.get('name', 'Nouveau') == 'Nouveau':
			...
			if not annee:
				raise ValidationError(_("Veuillez configurer l'année scolaire"))
			vals = {
	...
	def write(self, values):
		...
		if 'classe' in values:
			...
			if inscrit:
				inscrit.classe_id = values['classe'],
			else:
				self.env['gescoth.eleve.inscription'].create(vals)
		return res
